...
```bash
    $ packer build -var 'perform_dist_upgrade=false' ubuntu1204.json
```

The operation 'apt-get dist-upgrade' doesn't perform update of distribution to the next e.g. 12.10 or 14.04 LTS but only upgrade some additional packages (usually related to Linux kernel itself).
